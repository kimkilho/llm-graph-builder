...
async def get_graph_document_list(
    ...
    if "diffbot_api_key" in dir(llm):
        ...
        ignore_tool_usage = not any(
            pattern in model_name for pattern in TOOL_SUPPORTED_MODELS
        )
...
async def get_graph_from_llm(
    ...
    try:
        ...
        allowed_nodes = [
            node.strip() for node in allowedNodes.split(",") if node.strip()
        ]
        ...
        if allowedRelationship:
            items = [
                item.strip() for item in allowedRelationship.split(",") if item.strip()
            ]
            if len(items) % 3 != 0:
