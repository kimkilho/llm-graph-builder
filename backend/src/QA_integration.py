...
class SessionChatHistory:
    ...
    def get_chat_history(cls, session_id):
        ...
        if session_id not in cls.history_dict:
            logging.info(
                f"Creating new ChatMessageHistory Local for session ID: {session_id}"
            )
            cls.history_dict[session_id] = ChatMessageHistory()
        else:
            logging.info(
                f"Retrieved existing ChatMessageHistory Local for session ID: {session_id}"
            )
...
def get_total_tokens(ai_response, llm):
    try:
        ...
        if isinstance(llm, (ChatOpenAI, AzureChatOpenAI, ChatGroq)):
            total_tokens = ai_response.response_metadata.get("token_usage", {}).get(
                "total_tokens", 0
            )

        elif isinstance(llm, ChatVertexAI):
            total_tokens = ai_response.response_metadata.get("usage_metadata", {}).get(
                "prompt_token_count", 0
            )

        elif isinstance(llm, ChatBedrock):
            total_tokens = ai_response.response_metadata.get("usage", {}).get(
                "total_tokens", 0
            )

        elif isinstance(llm, ChatAnthropic):
            input_tokens = int(
                ai_response.response_metadata.get("usage", {}).get("input_tokens", 0)
            )
            output_tokens = int(
                ai_response.response_metadata.get("usage", {}).get("output_tokens", 0)
            )
            ...
            logging.warning(
                f"Unrecognized language model: {type(llm)}. Returning 0 tokens."
            )
            total_tokens = 0
        ...
        try:
            ...
            if "entities" in doc.metadata:
                if chat_mode_settings["mode"] == CHAT_ENTITY_VECTOR_MODE:
                    entity_ids = [
                        entry["entityids"]
                        for entry in doc.metadata["entities"]
                        if "entityids" in entry
                    ]
                    entities.setdefault("entityids", set()).update(entity_ids)
                else:
                    if "entityids" in doc.metadata["entities"]:
                        entities.setdefault("entityids", set()).update(
                            doc.metadata["entities"]["entityids"]
                        )
            ...
            if "communitydetails" in doc.metadata:
                ...
                new_entries = [
                    entry
                    for entry in doc.metadata["communitydetails"]
...
def initialize_neo4j_vector(graph, chat_mode_settings):
    try:
        ...
        if not retrieval_query or not index_name:
            raise ValueError(
                "Required settings 'retrieval_query' or 'index_name' are missing."
            )
...
def create_retriever(
    neo_db, document_names, chat_mode_settings, search_k, score_threshold, ef_ratio
):
    if document_names and chat_mode_settings["document_filter"]:
        ...
        logging.info(
            f"Successfully created retriever with search_k={search_k}, score_threshold={score_threshold}"
        )
    return retriever
...
def get_neo4j_retriever(
    ...
    try:
        ...
        ef_ratio = (
            int(os.getenv("EFFECTIVE_SEARCH_RATIO", "2"))
            if os.getenv("EFFECTIVE_SEARCH_RATIO", "2").isdigit()
            else 2
        )
...
def process_chat_response(
    messages, history, question, model, graph, document_names, chat_mode_settings
):
    try:
        ...
        summarization_thread = threading.Thread(
            target=summarize_and_log, args=(history, messages, llm)
        )
        ...
        logging.exception(
            f"Error processing chat response at {datetime.now()}: {str(e)}"
        )
        return {
            "session_id": "",
...
def process_graph_response(model, graph, question, messages, history):
    try:
        ...
        summarization_thread = threading.Thread(
            target=summarize_and_log, args=(history, messages, qa_llm)
        )
        summarization_thread.start()
        logging.info("Summarization thread started.")
        metric_details = {
        ...
        logging.exception(
            f"Error processing graph response at {datetime.now()}: {str(e)}"
        )
        return {
            "session_id": "",
            "message": "Something went wrong",
